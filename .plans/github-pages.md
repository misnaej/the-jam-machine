...
### Page 3: Track Analysis — Reptilia Case Study (`track-analysis.html`)

An interactive HTML report generated from the Reptilia MIDI, showcasing what the encoding pipeline sees. This makes the abstract encoding concepts concrete.

**Visualizations to include:**

1. **Piano roll (full piece)** — already generated by `plot_piano_roll()`. Color-coded by instrument, bar lines visible. Shows the complete musical structure at a glance.

2. **Per-instrument timeline** — horizontal bars showing when each instrument plays (start→end), with note density as opacity/color. Immediately shows arrangement structure (intro, verse, chorus).

3. **Split instrument visualization** — already implemented. Shows where a single MIDI instrument is split across multiple tracks (common in guitar parts with overdubs).

4. **Note density heatmap** — X: beats (in bars), Y: instruments. Color = notes per bar. Shows which instruments are active when, and how dense the arrangement is.

5. **Pitch distribution per instrument** — histogram or violin plot showing the pitch range each instrument uses. Drums will cluster at specific GM drum map pitches; bass will be low; guitars will be mid-range.

6. **Encoding comparison table** — side-by-side: original MIDI stats vs encoded text stats vs decoded MIDI stats. Shows what's preserved and what's lost (timing resolution, instrument families, note counts).

7. **Token vocabulary breakdown** — bar chart showing token frequency in the encoded text. How many NOTE_ON vs TIME_DELTA vs BAR_START etc. Gives a feel for the "language" the model learns.

8. **Quantization impact** — before/after comparison of a short passage. Show the original precise timing vs the quantized version. Highlight where notes were merged (zero time deltas discarded).

**Format:** Generate as a self-contained HTML file using matplotlib + a simple HTML template. The `track_stats.py` example already produces the data; extend it to generate a full report. Could use Jinja2 for templating or just embed base64 PNGs in HTML.

**Generation script:** `scripts/build-track-analysis.sh` or a Python script in examples/ that produces `docs/track-analysis.html`.

---

### Page 4: Embedding Explorer (`exploring_the_embedding.html`)
